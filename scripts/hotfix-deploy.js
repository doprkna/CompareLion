...
function exec(command, options = {}) {
    ...
    try {
        ...
        console.error(`âŒ Command failed: ${command}`);
        throw error;
    }
}
/**
...
function updatePackageVersion(newVersion) {
    ...
    console.log(`\nðŸ“¦ Version: ${packageJson.version} â†’ ${newVersion}`);
...
async function main() {
    ...
    if (!description) {
        console.error('âŒ Error: Hotfix description required');
        console.log('\nUsage:');
    ...
    console.log('ðŸš€ HOTFIX DEPLOYMENT PIPELINE');
    ...
    try {
        ...
        // 3. Update version
        console.log('\nðŸ“‹ Step 2: Bumping version...');
        updatePackageVersion(newVersion);
        // 4. Update changelog
        console.log('\nðŸ“‹ Step 3: Updating changelog...');
        updateChangelog(newVersion, description);
        // 5. Lint
        console.log('\nðŸ“‹ Step 4: Running linter...');
        try {
        ...
        // 7. Run tests
        console.log('\nðŸ“‹ Step 6: Running tests...');
        try {
            exec('pnpm test', { stdio: 'pipe' });
            console.log('âœ… Tests passed');
        }
        catch (error) {
            console.error('âŒ Tests failed');
        ...
        console.error('âŒ HOTFIX DEPLOYMENT FAILED');
        console.error('âŒ â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•â•\n');
        console.error(error);
        process.exit(1);
    }
}
