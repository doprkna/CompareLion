...
import { getAllModelNames, checkModelIntegrity, generateSummary, } from '../apps/web/lib/db/integrity-utils';
...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < modelNames.length; i += chunkSize) {
            ...
            const chunkResults = await Promise.all(chunk.map(async (modelName) => {
                try {
