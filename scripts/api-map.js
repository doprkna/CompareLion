 ...
 */
function checkComments(content) {
    return {
 ...
 */
function filePathToApiPath(filePath, baseDir) {
 ...
 */
async function scanFeUsage(apiPath, webDir) {
 ...
 */
async function findFeFiles(dir) {
    const files = [];
    try {
        const entries = await readdir(dir, { withFileTypes: true });
        for (const entry of entries) {
 ...
 */
function getSchemaModels(schemaPath) {
    try {
 ...
 */
function groupRoutesBySystem(routes) {
    const grouped = {};
