...
import { generateStoryPanels, generateExtendedStory } from './storyService';
import { createNotification } from '@parel/notifications';
/**
 * Get story panels for remix source
 */
export async function getStoryPanels(storyId) {
    try {
 ...
 */
async function getRemixChainDepth(storyId) {
    let depth = 0;
 ...
 * Create remix story
 */
export async function createRemixStory(userId, parentStoryId, newPanelImages, newPanelTexts) {
    try {
        ...
        const combinedPanelTexts = [
            ...parentPanels.map((p) => p.text || null),
            ...normalizedNewTexts,
        ];
        // Determine remix type based on number of new panels
        const remixType = newPanelImages.length === 1 ? 'response' : newPanelImages.length >= 2 ? 'extend' : 'alt';
        // Generate story (use extended story generator for remixes)
        const totalPanels = combinedPanelImages.length;
        const story = totalPanels <= 3
            ? await generateStoryPanels(combinedPanelImages, combinedPanelTexts, totalPanels === 1 ? '1panel' : '3panel')
 ...
 * Get remix metadata
 */
export async function getRemixMetadata(storyId) {
    try {
        const story = await prisma.story.findUnique({
            where: { id: storyId },
            include: {
 ...
 */
export async function getRemixCount(parentStoryId) {
    try {
