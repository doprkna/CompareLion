 ...
 */
export async function generateStoryPanels(panelImages, panelTexts, mode) {
    try {
        ...
        // Generate panels
        const panels = await Promise.all(panelImages.map(async (imageUrl, index) => {
 ...
 */
async function callAIForPanel(imageUrl, text, category) {
 ...
 */
async function callAIForThroughline(panels) {
    if (!GEN_CONFIG.GPT_URL || !GEN_CONFIG.GPT_KEY) {
 ...
 */
export async function generateExtendedStory(panelImages, panelTexts, requestIds) {
    try {
        ...
        const categories = await Promise.all(panelImages.map(async (imageUrl) => {
            try {
                const detection = await detectImageCategory(imageUrl);
        ...
        const story = await callAIForExtendedStory(panelImages, panelTexts, categories, aureContexts);
        return story;
    }
    catch (error) {
 ...
 */
async function callAIForExtendedStory(panelImages, panelTexts, categories, aureContexts) {
    if (!GEN_CONFIG.GPT_URL || !GEN_CONFIG.GPT_KEY) {
        // Fallback
...
Return JSON:
{
  ...
  "panels": [
    ...
    ...
        ...
        const panels = parsed.panels.slice(0, panelImages.length).map((panel, index) => ({
            role: (panel.role || roleMap[index]),
 ...
 */
function mapPanelsToRoles(numPanels) {
    const roles = [];
    if (numPanels === 4) {
