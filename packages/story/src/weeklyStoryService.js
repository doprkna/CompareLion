    ...
    try {
        ...
        try {
            ...
            questHighlights = questCompletions
                ...
                .map((completion) => completion.quest.title || 'Quest completed');
        }
        catch (error) {
 ...
 */
export async function generateWeeklyStory(data, userId) {
    try {
        ...
        let numPanels = 3;
        if (data.topImages.length >= 2)
            numPanels = 4;
        if (data.archetypeChange)
            numPanels = Math.min(6, numPanels + 1);
        if (data.questHighlights.length > 0)
            numPanels = Math.min(6, numPanels + 1);
        if (data.weirdMoments.length > 0)
            numPanels = Math.min(6, numPanels + 1);
 ...
 */
async function callAIForWeeklyStory(data, numPanels) {
...
Make it celebratory and fun!`;
        const topCategories = Object.entries(data.categoryDistribution)
            ...
            .join(', ');
        const imageDescriptions = data.topImages.slice(0, numPanels).map((img, idx) => `Panel ${idx + 1}: ${img.category} - ${img.caption || 'image'}`).join('\n');
