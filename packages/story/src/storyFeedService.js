...
import { computeStoryRankScore } from './storyRankingService';
 ...
 */
export async function getPublicStoriesFeed(params = {}) {
    ...
    try {
        ...
        const stickerCountsMap = new Map(); // storyId -> stickerId -> count
        storyIds.forEach((id) => {
        ...
        reactions.forEach((r) => {
            ...
            if (stickerId) {
                ...
                if (r.type === 'like')
                    counts.like = r._count.id;
                if (r.type === 'lol')
                    counts.lol = r._count.id;
                if (r.type === 'vibe')
                    counts.vibe = r._count.id;
        ...
        const storyChallengeMap = new Map();
        ...
        const feedItemsWithScores = stories.map((story) => {
 ...
 */
export async function handleStoryReaction(userId, storyId, type, action = 'toggle') {
    try {
 ...
 */
export async function getStoryReactionSummary(storyId) {
    const reactions = await prisma.storyReaction.groupBy({
        by: ['type'],
        where: {
            storyId,
    ...
    reactions.forEach((r) => {
        ...
        if (stickerId) {
            ...
            if (r.type === 'like')
                reactionCounts.like = r._count.id;
            if (r.type === 'lol')
                reactionCounts.lol = r._count.id;
            if (r.type === 'vibe')
                reactionCounts.vibe = r._count.id;
        }
    });
    const stickers = Array.from(stickerCounts.entries())
        .map(([stickerId, count]) => {
        const sticker = getStickerById(stickerId);
        if (!sticker)
            return null;
 ...
 */
export async function toggleStoryReaction(userId, storyId, type) {
 ...
 */
export async function publishStory(userId, storyId, visibility) {
    try {
        // Verify ownership
        const story = await prisma.story.findUnique({
            where: { id: storyId },
 ...
 */
export async function updateStoryVisibility(userId, storyId, visibility) {
    try {
        // Verify ownership
        const story = await prisma.story.findUnique({
            where: { id: storyId },
