 ...
 */
export function computeStoryRankScore(story, activeChallengeIds = []) {
    try {
        // A) Recency Boost
        const now = new Date();
        ...
        // B) Reaction Total
        const reactionTotal = (story.reactions.like || 0) +
        ...
        if (story.inChallenges && story.inChallenges.length > 0) {
            ...
            const hasActiveChallenge = story.inChallenges.some((challengeId) => activeChallengeIds.includes(challengeId));
...
export async function getActiveChallengeIds() {
    // Try cache first
    const cached = await cacheGet('challenge:active-ids');
    if (cached !== null) {
        return cached;
    }
