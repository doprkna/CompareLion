 ...
 */
export async function incrementStoryView(storyId, userId) {
    try {
        // Verify story exists
 ...
 */
export async function getStoryReactionStats(storyId) {
    try {
 ...
 */
export async function getStoryStickerStats(storyId) {
    try {
        const summary = await getStoryReactionSummary(storyId);
 ...
 */
export async function getStoryChallenges(storyId) {
    try {
 ...
 */
export async function calculateStoryReachScore(storyId) {
    try {
        const story = await prisma.story.findUnique({
            where: { id: storyId },
            select: { viewCount: true },
        ...
        const reachScore = story.viewCount + reactionsTotal * 3 + stickersTotal * 2 + challengeBonus;
