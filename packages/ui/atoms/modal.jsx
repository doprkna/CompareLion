...
export function Modal({ open, onClose, children, title, size = 'md', closeOnBackdrop = true, closeOnEsc = true, header, footer, className = '', }) {
    ...
    useEffect(() => {
        ...
        const handleTab = (e) => {
            if (e.key !== 'Tab' || !modalRef.current)
                return;
            const focusableElements = Array.from(modalRef.current.querySelectorAll('button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])'));
    ...
    const modalContent = (<div className={backdropStyles} onClick={handleBackdropClick} role="dialog" aria-modal="true">
      <div ref={modalRef} className={modalStyles} onClick={(e) => e.stopPropagation()}>
        {(title || header) && (<div className="px-6 py-4 border-b border-gray-200">
            {header || (<div className="flex items-center justify-between">
                <h2 className="text-lg font-semibold text-gray-900">{title}</h2>
                <button onClick={onClose} className="text-gray-400 hover:text-gray-600 focus:outline-none focus:ring-2 focus:ring-blue-500 rounded" aria-label="Close modal">
