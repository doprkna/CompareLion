...
export function Checkbox({ checked = false, onChange, disabled = false, indeterminate = false, label, className = '', id, ...props }) {
    ...
    const checkbox = (<input ref={checkboxRef} id={checkboxId} type="checkbox" checked={checked} onChange={onChange} disabled={disabled} aria-checked={indeterminate ? 'mixed' : checked} className={combinedStyles} {...props}/>);
    if (label) {
