...
export function Switch({ checked = false, onChange, disabled = false, label, size = 'md', className = '', id, showCheckIcon = false, ...props }) {
    ...
    const switchElement = (<label htmlFor={switchId} className={combinedTrackStyles}>
      <input id={switchId} type="checkbox" role="switch" checked={checked} onChange={onChange} disabled={disabled} aria-checked={checked} className="sr-only" {...props}/>
      <span className={`${combinedThumbStyles} ${showCheckIcon && checked ? 'flex items-center justify-center' : ''}`}>
        {showCheckIcon && checked && (<Icon name="check" size={12} className="text-blue-600" aria-hidden="true"/>)}
      </span>
    </label>);
    if (label) {
