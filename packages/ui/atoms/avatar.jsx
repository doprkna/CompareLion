...
function getInitials(fallback, alt) {
    if (fallback) {
...
export function Avatar({ src, alt = '', size = 'md', fallback, status, className = '', }) {
    ...
    return (<div className={combinedStyles}>
      {showImage ? (<img src={src} alt={alt} className="w-full h-full object-cover" onError={() => setImageError(true)}/>) : fallback || initials !== '?' ? (<span>{initials}</span>) : (<Icon name="user" size={size === 'sm' ? 'sm' : size === 'xl' ? 'lg' : 'md'} className="text-gray-400" aria-hidden="true"/>)}
      {status && (<span className={`absolute bottom-0 right-0 rounded-full border-2 border-white ${statusStyles[status]} ${statusSizeStyles[size]}`} aria-label={`Status: ${status}`}/>)}
    </div>);
}
