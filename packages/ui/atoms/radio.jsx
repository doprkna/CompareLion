...
export function Radio({ checked = false, onChange, name, value, disabled = false, label, className = '', id, ...props }) {
    ...
    const radio = (<input id={radioId} type="radio" name={name} value={value} checked={checked} onChange={onChange} disabled={disabled} aria-checked={checked} className={combinedStyles} {...props}/>);
    if (label) {
