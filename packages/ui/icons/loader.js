...
const missingIconWarnings = new Set();
/**
 ...
 */
export async function loadIcon(name, variant) {
    // Create cache key
    ...
    if (!ICONS[name]) {
        if (process.env.NODE_ENV === 'development') {
            ...
            if (!missingIconWarnings.has(warningKey)) {
                console.warn(`[Icon] Icon not found in registry: "${name}"${variant ? ` (variant: ${variant})` : ''}. Using fallback.`);
    ...
    try {
        ...
        if (!IconComponent) {
            if (process.env.NODE_ENV === 'development') {
                const warningKey = `${name}${variant ? `-${variant}` : ''}`;
                if (!missingIconWarnings.has(warningKey)) {
                    console.warn(`[Icon] Icon "${name}" is not yet implemented (stub). Using fallback.`);
                    missingIconWarnings.add(warningKey);
                }
            }
            iconCache.set(cacheKey, FallbackIcon);
