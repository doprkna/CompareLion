 ...
 */
function createIconWrapper(LucideIcon) {
    return ({ size, className, 'aria-label': ariaLabel, 'aria-hidden': ariaHidden, ...props }) => {
        // Convert size to number if it's a string
        const iconSize = typeof size === 'string' ? parseInt(size, 10) || 24 : size || 24;
        return (<LucideIcon size={iconSize} className={className} aria-label={ariaLabel} aria-hidden={ariaHidden} {...props}/>);
    };
}
/**
