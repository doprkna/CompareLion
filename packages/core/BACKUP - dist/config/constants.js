 ...
 */
export function xpToCoins(xp) {
    // Use unified config instead of constants
    // Dynamic import to avoid circular dependency
    const config = require('./index').config;
    return Math.floor(xp / config.gameplay.economy.xpToCoinsRatio);
}
/**
 * @deprecated Use config.gameplay.economy.xpToCoinsRatio instead
...
export function coinsToXP(coins) {
    // Use unified config instead of constants
    const config = require('./index').config;
    return coins * config.gameplay.economy.xpToCoinsRatio;
}
/**
 * @deprecated Use config.gameplay.economy.rewards instead
 * Get coin reward for an action
 */
export function getCoinReward(action) {
    // Use unified config instead of constants
    const config = require('./index').config;
    return config.gameplay.economy.rewards[action] || 0;
}
/**
 * @deprecated Use config.gameplay.economy.cosmeticPricing instead
 * Get pricing range for a cosmetic rarity
 */
export function getPriceRange(rarity) {
    // Use unified config instead of constants
    const config = require('./index').config;
    return config.gameplay.economy.cosmeticPricing[rarity];
}
/**
 * @deprecated Use config.gameplay.leaderboard.seasonEndRewards instead
 * Get season end reward for a rank
 */
export function getSeasonEndReward(rank) {
    // Use unified config instead of constants
    const config = require('./index').config;
    const rewards = config.gameplay.leaderboard.seasonEndRewards;
    if (rank === 1)
        return rewards[1];
    if (rank === 2)
        return rewards[2];
