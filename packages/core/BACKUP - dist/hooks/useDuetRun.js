...
export function useDuetRun() {
    const [duetRun, setDuetRun] = useState(null);
    ...
    const load = useCallback(async () => {
        ...
        try {
            ...
            if (!res.ok || !json?.success)
                throw new Error(json?.error || 'Failed to load duet run');
...
export function useStartDuetRun() {
    ...
    const [error, setError] = useState(null);
    const start = useCallback(async (missionKey, partnerId) => {
        setLoading(true);
        setError(null);
        try {
            ...
            const json = await res.json();
            if (!res.ok || !json?.success)
                throw new Error(json?.error || 'Failed to start duet run');
            return json;
        }
        catch (e) {
...
export function useDuetProgress() {
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState(null);
    const updateProgress = useCallback(async (duetRunId, progress) => {
        setLoading(true);
        setError(null);
        try {
    ...
    const complete = useCallback(async (duetRunId) => {
        setLoading(true);
        setError(null);
        try {
            ...
            const json = await res.json();
            if (!res.ok || !json?.success)
                throw new Error(json?.error || 'Failed to complete duet run');
            return json;
        }
        catch (e) {
