...
const useSWR = (key, fetcher, options) => ({ data: null, error: null, isLoading: false, mutate: () => { } });
const useSWRInfinite = (getKey, fetcher, options) => ({ data: null, error: null, isLoading: false, mutate: () => { }, setSize: () => { }, size: 0 });
...
export function useMarketTransactions(limit = 3) {
    const { data, error, isLoading, mutate } = useSWR(`/api/market/transactions?limit=${limit}`, fetcher, {
        revalidateOnFocus: false,
        onError: (err) => {
            if (err instanceof Error) {
                const status = err.status;
 ...
 */
export function useEconomySummary(withTrends = false) {
 ...
 */
export function useEconomyTrends() {
...
export function useBalanceSettings() {
    ...
    const updateSetting = async (key, value) => {
        try {
            ...
            const optimisticSettings = currentSettings.map((s) => s.key === key ? { ...s, value, updatedAt: new Date().toISOString() } : s);
 ...
 */
export function useEconomyPresets() {
 ...
 */
export function useAdminEconomyActions() {
 ...
 */
export function useSystemAlerts(openOnly = true) {
...
export function useEconomyModifiers() {
    ...
    const setWeeklyModifier = async (presetIndex) => {
        try {
            const res = await fetch('/api/admin/economy/modifiers', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
...
export function useFeaturedItems() {
    const { data, error, isLoading, mutate } = useSWR('/api/market/items?isFeatured=true&limit=5', fetcher, {
        revalidateOnFocus: false,
        revalidateOnReconnect: false,
        refreshInterval: 5 * 60 * 1000, // Refresh every 5 minutes
        dedupingInterval: 60000, // 1 minute deduplication
 ...
 */
export function useMountTrials() {
    ...
    const updateProgress = async (trialId, incrementBy = 1) => {
        try {
 ...
 */
export function useMarket(category) {
    const url = category
