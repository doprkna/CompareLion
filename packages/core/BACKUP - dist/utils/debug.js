 ...
 */
export function debugIf(condition, message, context) {
 ...
 * Test-only logging
 */
export function testLog(message, context) {
    if (IS_TEST) {
 ...
 */
export function logApi(method, path, status, duration) {
    if (!DEBUG_ENABLED)
        return;
 ...
 */
export function logQuery(operation, table, duration) {
    if (!DEBUG_ENABLED)
        return;
    if (duration !== undefined) {
