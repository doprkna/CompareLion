/**
 * Latest Lore Store
 * Zustand store for latest lore entries collection
 * v0.41.19 - C3 Step 20: State Migration Batch #3
 */

'use client';

import { createAsyncStore } from '../factory';
import { defaultClient } from '@parel/api'; // sanity-fix: replaced @parel/api/client with @parel/api (client not exported as subpath)
import type { LoreEntry } from './loreStore';

interface LatestLoreData {
  entries: LoreEntry[];
}

export const useLatestLoreStore = createAsyncStore<LatestLoreData>({
  name: 'latestLore',
  fetcher: async () => {
    const response = await defaultClient.get<LatestLoreData>('/lore/latest');
    return response.data;
  },
  cacheTtl: 5 * 60 * 1000, // 5 minutes
});

