...
interface SubmitMirrorReflectionStoreState {
  loading: boolean;
  error: string | null;
  submit: (
    mirrorEventId: string,
    answers: Array<{ questionIndex: number; content: string }>
  ) => Promise<any>;
}

export const useSubmitMirrorReflectionStore = createStore<SubmitMirrorReflectionStoreState>(
  (set, get) => ({
    loading: false,
    error: null,
