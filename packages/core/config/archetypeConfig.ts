...
export interface ArchetypeDefinition {
  ...
  emoji: string;
  baseStats: {
    str: number; // Strength - affects physical damage, HP
    int: number; // Intelligence - affects XP gain, reflection rewards
    cha: number; // Charisma - affects social bonuses, shop discounts
    luck: number; // Luck - affects crit chance, drop rates
  };
  growthRates: {
    str: number;
    int: number;
  ...
  bonuses: {
    combat?: {
      damageMult?: number; // Damage multiplier
      critChance?: number; // Additional crit chance
      hpMult?: number; // HP multiplier
    };
    xp?: {
      reflectionBonus?: number; // Bonus XP from reflections
      generalBonus?: number; // General XP bonus
    };
    social?: {
      shopDiscount?: number; // Shop discount percentage (0-1)
      socialBonus?: number; // Social interaction bonuses
    };
    passive?: {
      regen?: number; // HP regen per turn
      goldBonus?: number; // Gold earning bonus
    };
  };
}
 ...
 */
export function getArchetype(key: string): ArchetypeDefinition | null {
...
export function calculateStatsForLevel(
  ...
  level: number
): { str: number; int: number; cha: number; luck: number } {
