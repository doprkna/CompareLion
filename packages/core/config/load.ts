 ...
 */
function getEnvironmentConfig(): PartialUnifiedConfig {
 ...
 */
function getRuntimeOverrides(): PartialUnifiedConfig {
  const env = process.env.NODE_ENV || 'development';
 ...
 */
const REQUIRED_FIELDS: Array<{ path: string; namespace: keyof UnifiedConfig }> = [
 ...
 */
function enforceRequiredFields(config: Partial<UnifiedConfig>): void {
  ...
  if (missing.length > 0) {
    throw new Error(
      `[Config] Required config namespaces are missing: ${missing.join(', ')}\n` +
 ...
 */
export function loadConfig(userOverrides?: PartialUnifiedConfig): Readonly<UnifiedConfig> {
  // Step 1: Start with base defaults
  let config = { ...unifiedConfigDefaults } as UnifiedConfig;
  ...
  if (!validation.valid) {
    ...
    throw new Error(
      `[Config] Config validation failed with ${validation.errors.length} error(s). ` +
  ...
  return frozenConfig as Readonly<UnifiedConfig>;
}
