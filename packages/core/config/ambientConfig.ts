...
export interface AmbientScene {
  gradient: [string, string, string]; // CSS gradient stops (3 colors)
  sfx: string | null; // Audio loop filename (null = no audio)
  particles: 'embers' | 'sparks' | 'coins' | 'glow' | 'none';
  transitionDuration?: number; // Optional override (default 1.5s)
}

export const AMBIENT_SCENES: Record<AmbientMode, AmbientScene> = {
  combat: {
 ...
 */
export function getAmbientScene(mode: AmbientMode): AmbientScene {
