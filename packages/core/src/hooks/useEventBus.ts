 ...
 */
export function useEventBus(event: string, handler: (payload: any) => void) {
 ...
 * @param handler Callback function to execute when event is emitted
 */
export function useEventBusOnce(event: string, handler: (payload: any) => void) {
  const stableHandler = useCallback(handler, [handler]);
 ...
 */
export function useEventBusMultiple(events: Record<string, (payload: any) => void>) {
  useEffect(() => {
    if (!events || typeof events !== 'object') return; // sanity-fix
