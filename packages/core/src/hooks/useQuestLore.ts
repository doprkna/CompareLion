...
export function useQuestLore(questId: string | null) {
  ...
  const fetchLore = async () => {
    ...
    setError(null);
    try {
      const response = await defaultClient.get<QuestsResponseDTO>('/quests?includeLore=true');
      
      const quest = response?.data?.quests?.find((q: any) => q.questId === questId); // sanity-fix
      if (quest?.lore) {
        setLore(quest.lore);
      } else {
        setLore(null);
