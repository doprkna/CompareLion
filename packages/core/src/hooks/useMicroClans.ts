...
export function useMicroClans(seasonId?: string) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json?.success) throw new Error(json?.error || 'Failed to load clans');
      setClans(json.clans || []);
    } catch (e: any) {
...
export function useClan(clanId: string | null) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json?.success) throw new Error(json?.error || 'Failed to load clan');
      setClan(json.clan);
    } catch (e: any) {
...
export function useClanBuff() {
  const [buff, setBuff] = useState<any | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const activeClan = userClans.find((c: any) => c.buffActive);
