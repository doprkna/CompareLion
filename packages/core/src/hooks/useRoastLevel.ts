...
export function useRoastLevel() {
  const [roastLevel, setRoastLevel] = useState<number>(3);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json?.success) throw new Error(json?.error || 'Failed to load roast level');
...
export function useSetRoastLevel() {
  ...
  const setLevel = useCallback(async (level: number) => {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok || !json?.success) throw new Error(json?.error || 'Failed to set roast level');
      return json;
    } catch (e: any) {
