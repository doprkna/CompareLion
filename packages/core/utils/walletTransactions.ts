...
const prisma = {
  $transaction: async (fn: any) => fn({ wallet: { findFirst: async () => null, update: async () => ({ funds: { toNumber: () => 0 }, diamonds: 0 }) }, ledgerEntry: { createMany: async () => {}, create: async () => {} } })
} as any;
 ...
 */
export async function updateWalletWithLock(
  update: WalletUpdate
 ...
 */
export async function getWalletBalanceWithLock(
  userId: string,
  tenantId: string
...
export async function transferBetweenWallets(
  ...
  tenantId: string,
  fundsAmount: number,
  diamondsAmount: number = 0,
  refType: string = 'transfer',
  note?: string
