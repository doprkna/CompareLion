...
export function useSfx(
  ...
  const play = (key: string, volume: number = 0.4) => {
    ...
    const now = Date.now();
    if (!playDebounceMap || typeof playDebounceMap.get !== 'function') return; // sanity-fix
    ...
    try {
      if (typeof window === 'undefined' || typeof Audio === 'undefined') return; // sanity-fix
      if (!audioCache.current || typeof audioCache.current.get !== 'function') return; // sanity-fix
  ...
  const vibrate = (pattern?: number[]) => {
    if (!hapticsEnabled || typeof navigator === 'undefined' || !navigator.vibrate) return; // sanity-fix
