...
const CAT_IDS = ['seed-demo-cat-1', 'seed-demo-cat-2'] as const;
const CAT_NAMES: Record<string, string> = { 'seed-demo-cat-1': 'Wellbeing', 'seed-demo-cat-2': 'Habits' };
const DEMO_EMAIL = 'demo@example.com';
const LOCALE = 'en';

function qId(catIdx: number, qIdx: number): string {
  return `seed-demo-q-${catIdx + 1}-${String(qIdx + 1).padStart(2, '0')}`;
}
function optId(catIdx: number, qIdx: number, optIdx: number): string {
...
const QUESTIONS: { text: string; type: QuestionType; options?: string[] }[] = [
...
async function seedDemoUser(): Promise<boolean> {
  ...
  await prisma.user.upsert({
    ...
    update: { xp: 0, streakCount: 0, level: 1, birthYear: 1990 },
  });
  return true;
}
