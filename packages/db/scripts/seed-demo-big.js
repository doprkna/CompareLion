...
function ssscId(i) { return `seed-big-sssc-${String(i + 1).padStart(2, '0')}`; }
function qId(catIdx, qIdx) { return `seed-big-q-${String(catIdx + 1).padStart(2, '0')}-${String(qIdx + 1).padStart(3, '0')}`; }
function optId(catIdx, qIdx, optIdx) { return `seed-big-opt-${String(catIdx + 1).padStart(2, '0')}-${String(qIdx + 1).padStart(3, '0')}-${optIdx + 1}`; }
...
async function seedCategories(stats) {
...
async function seedFlowQuestions(catIds, rng, stats) {
    const questions = [];
    ...
    for (const q of questions) {
        await prisma.flowQuestion.upsert({
            where: { id: q.id },
            update: { text: q.text, type: q.type, categoryId: q.categoryId, isActive: true },
            create: { id: q.id, text: q.text, type: q.type, categoryId: q.categoryId, isActive: true },
        });
        if (q.optionIds) {
...
async function seedUsers(rng, stats) {
    ...
    for (let i = 0; i < 50; i++) {
        ...
        const u = await prisma.user.upsert({
            where: { email },
            update: { passwordHash: pwHash, name: `Demo User ${i + 1}`, xp: 0, level: 1, streakCount: 0, questionsAnswered: 0 },
            create: {
                email, passwordHash: pwHash, name: `Demo User ${i + 1}`,
                xp: 0, level: 1, streakCount: 0, questionsAnswered: 0, role: UserRole.USER,
            },
        });
        ids.push(u.id);
    }
...
async function seedResponses(userIds, questions, rng, stats) {
    let created = 0;
    for (const uid of userIds) {
        const used = new Set();
        for (let r = 0; r < 10; r++) {
            let idx = Math.floor(rng() * questions.length);
            for (let retry = 0; retry < 50 && used.has(idx); retry++)
                idx = Math.floor(rng() * questions.length);
            used.add(idx);
