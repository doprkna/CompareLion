...
export const POST = safeAsync(async (req) => {
    ...
    const inactiveAffinities = await prisma.npcAffinity.findMany({
        where: {
    ...
    for (const affinity of inactiveAffinities) {
        const daysSinceInteraction = Math.floor((now.getTime() - affinity.lastInteraction.getTime()) / (24 * 60 * 60 * 1000));
        ...
        if (newScore < affinity.affinityScore) {
            await prisma.npcAffinity.update({
    ...
    const removed = await prisma.npcAffinity.deleteMany({
        where: {
