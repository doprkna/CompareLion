...
export const GET = safeAsync(async (_req) => {
    const now = new Date();
    const windowEnd = now;
    ...
    for (const q of recent) {
        ...
        await prisma.trendingQuestion.upsert({
            where: { questionId_region: { questionId: q.id, region: (q.region || 'GLOBAL').toUpperCase() } },
    ...
    for (const g of topGlobal) {
        await prisma.trendingQuestion.upsert({
            where: { questionId_region: { questionId: g.id, region: 'GLOBAL' } },
