...
export const POST = safeAsync(async (req) => {
    ...
    const recentLogs = await prisma.userMoodLog.findMany({
        where: {
    ...
    const existingMood = await prisma.globalMood.findFirst({
        orderBy: { updatedAt: 'desc' },
    });
    if (existingMood) {
        // Update existing record
        await prisma.globalMood.update({
        ...
        // Create new record
        await prisma.globalMood.create({
            data: {
                calmScore,
                chaosScore,
                neutralScore,
    ...
    const purged = await prisma.userMoodLog.deleteMany({
        where: {
