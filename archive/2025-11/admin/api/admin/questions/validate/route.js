...
export async function POST(req) {
    try {
        ...
        const validations = {
            ...
            optionsValid: options ? options.every((opt) => opt.label && opt.value) : false,
        };
        ...
        if (!validations.hasText)
            feedback.push("Question text too short (min 10 chars)");
        if (!validations.hasQuestionMark)
            feedback.push("Question should end with ?");
        if (!validations.notTooLong)
            feedback.push("Question too long (max 200 chars)");
        if (!validations.hasOptions)
            feedback.push("Need at least 2 options");
        if (!validations.optionsValid)
            feedback.push("All options need label and value");
