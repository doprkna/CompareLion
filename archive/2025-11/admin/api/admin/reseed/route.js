...
export const POST = safeAsync(async (req) => {
    ...
    const successItems = [];
    if (result.stats.users > 0)
        successItems.push(`👥  users`);
    if (result.stats.achievements > 0)
        successItems.push(`🏆  achievements`);
    if (result.stats.items > 0)
        successItems.push(`📦  items`);
    if (result.stats.questions > 0)
        successItems.push(`❓  questions`);
    if (result.stats.messages > 0)
        successItems.push(`💬  messages`);
    if (result.stats.notifications > 0)
        successItems.push(`🔔  notifications`);
    if (result.stats.events > 0)
        successItems.push(`🌍  events`);
    if (result.stats.leaderboard > 0)
        successItems.push(`🏅  leaderboard entries`);
