...
import { parsePaginationParams, buildPaginationHeaders } from '@parel/core/utils/pagination';
export async function GET(req) {
    try {
        ...
        if (paginationParams.limit && paginationParams.limit > 1000) {
            return NextResponse.json({ success: false, error: 'Limit cannot exceed 1000' }, { status: 400 });
        }
        if (paginationParams.limit && paginationParams.limit < 1) {
            return NextResponse.json({ success: false, error: 'Limit must be at least 1' }, { status: 400 });
        }
        // Get audit logs
