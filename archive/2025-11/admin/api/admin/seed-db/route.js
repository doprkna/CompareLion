...
import { seedBadges } from "@/lib/seed-badges";
import { seedPotions } from "@/lib/rpg/seedPotions";
import { seedEvents } from "@/lib/rpg/seedEvents";
import { seedAdventureNodes } from "@/lib/rpg/seedAdventure";
import { seedCompanions } from "@/lib/rpg/seedCompanions";
// Utility to log audit
// Force Node.js runtime for Prisma (v0.35.16d)
export const runtime = 'nodejs';
async function logAudit(type, message, meta, userId) {
...
export async function POST() {
    try {
        ...
        // Seed badges (v0.36.24)
        await seedBadges();
        const badgesCount = await prisma.badge.count();
        // Seed potions (v0.36.14)
        const potionsCount = await seedPotions();
        // Seed events (v0.36.15)
        const eventsCount = await seedEvents();
        // Seed adventure nodes (v0.36.16)
        const adventureNodesCount = await seedAdventureNodes();
        // Seed companions (v0.36.17)
        const companionsCount = await seedCompanions();
        const duration = ((Date.now() - startTime) / 1000).toFixed(2);
        // Log audit
        await logAudit("admin:seed-db", `Seeder 2.0 executed via admin panel`, {
            users: users.length,
        ...
        return NextResponse.json({ error: error instanceof Error ? error.message : "Failed to seed database" }, { status: 500 });
    }
}
