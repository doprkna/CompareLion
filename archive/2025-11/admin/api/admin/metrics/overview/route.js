...
export const GET = safeAsync(async (req) => {
    ...
    const metricsData = await cached("admin-metrics-overview", 10 * 60 * 1000, async () => {
        const now = new Date();
        ...
        const [totalUsers, activeUsers, newUsersWeek, totalReflections, transactionsWeek, allUsers, recentTransactions,] = await Promise.all([
            // Total users
            prisma.user.count(),
        ...
        dates.forEach((date) => {
            ...
            const usersUpToDay = allUsers.filter((u) => new Date(u.createdAt) <= dayEnd).length;
