...
import { safeAsync, validationError, successResponse, parseBody, notFoundError } from '@/lib/api-handler';
 ...
 * Update a recipe
 */
export const PUT = safeAsync(async (req, { params }) => {
    await requireAdmin();
    ...
    if (validation.data.ingredients) {
        const materialIds = validation.data.ingredients.map((ing) => ing.materialId);
    ...
    if (validation.data.craftTime !== undefined)
        updateData.craftTime = validation.data.craftTime;
    if (validation.data.skillRequirement !== undefined)
        updateData.skillRequirement = validation.data.skillRequirement;
    if (validation.data.unlockLevel !== undefined)
        updateData.unlockLevel = validation.data.unlockLevel;
    if (validation.data.goldCost !== undefined)
        updateData.goldCost = validation.data.goldCost;
    try {
        ...
        return validationError(error instanceof Error ? error.message : 'Failed to update recipe');
    }
});
/**
 ...
 * Delete a recipe
 */
export const DELETE = safeAsync(async (req, { params }) => {
    await requireAdmin();
    const recipe = await prisma.recipe.findUnique({
        where: { id: params.id },
    });
    ...
    try {
        ...
        return validationError(error instanceof Error ? error.message : 'Failed to delete recipe');
    }
});
