...
/**
 * GET /api/admin/timezone/list
 * Get timezone system records
 */
export const GET = safeAsync(async (req: NextRequest) => {
  ...
  const results = await safePrismaListMultiple(
    [
      { name: 'userTimeZone', select: { id: true, userId: true, timeZone: true, offset: true, createdAt: true } },
      { name: 'regionSchedule', select: { id: true, regionId: true, schedule: true, createdAt: true } },
    ],
    limit
  );
