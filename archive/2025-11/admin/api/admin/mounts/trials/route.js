...
import { safeAsync, authError, validationError, notFoundError, successResponse, } from '@/lib/api-handler';
/**
 ...
 */
export const PATCH = safeAsync(async (req) => {
    const session = await getServerSession(authOptions);
    if (!session?.user?.email) {
        return authError('Unauthorized');
    }
    ...
    await prisma.$executeRawUnsafe(`UPDATE mount_trials SET ${updates.join(', ')} WHERE id = $${paramIndex}`, ...values, trialId);
 ...
 */
export const DELETE = safeAsync(async (req) => {
    const session = await getServerSession(authOptions);
    if (!session?.user?.email) {
        return authError('Unauthorized');
    }
