      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {fights.length === 0 ? (<div className="text-center py-12 text-muted-foreground">
                No fights found
              </div>) : (fights.map((fight) => (<Card key={fight.id} className="border">
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <div className="text-sm text-muted-foreground mt-1">
                          ...
                          <span className={fight.winner === "hero"
                ...
                : "text-red-600 font-semibold"}>
                      ...
                      <Button variant="ghost" size="sm" onClick={() => setExpandedFight(expandedFight === fight.id ? null : fight.id)}>
                        {expandedFight === fight.id ? (<ChevronUp className="h-4 w-4"/>) : (<ChevronDown className="h-4 w-4"/>)}
                      </Button>
                    </div>
                  </CardHeader>
                  {expandedFight === fight.id && (<CardContent>
                      <div className="space-y-2 max-h-64 overflow-y-auto">
                        {fight.rounds.map((round, idx) => (<div key={idx} className="p-2 rounded bg-muted/50 text-sm">
