...
import { FileText, ArrowLeft, RefreshCw, AlertCircle, CheckCircle, Info } from 'lucide-react';
export default function AdminLogs() {
    ...
    const getLevelBadge = (level) => {
        ...
        return (<Badge className={`${levelConfig[level]} text-xs`}>
        {level.toUpperCase()}
      </Badge>);
    };
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-4">
              {logs.map((log, index) => (<div key={index} className="flex items-start gap-4 p-4 border rounded-lg hover:bg-muted/50">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {log.details && (<p className="text-sm text-muted-foreground mt-1">
                        {log.details}
                      </p>)}
                  </div>
                </div>))}
