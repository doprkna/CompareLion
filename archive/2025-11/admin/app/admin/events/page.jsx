...
export default function AdminEventsPage() {
    ...
    async function toggleEventActive(eventId, currentActive) {
        try {
            ...
            if (res.ok) {
                ...
                throw new Error(res.error || 'Failed to toggle event');
            }
        }
        catch (error) {
    ...
    async function triggerEventNow(eventCode) {
        try {
            ...
            if (res.ok) {
                ...
                throw new Error(res.error || 'Failed to trigger event');
            }
        }
        catch (error) {
    ...
    async function seedEvents() {
        try {
            ...
            if (res.ok) {
                toast.success(`Seeded ${res.data?.count || 0} events`);
                loadEvents();
            }
            else {
                throw new Error(res.error || 'Failed to seed events');
            }
        }
        catch (error) {
    ...
    return (<div className="min-h-screen bg-bg p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card className="bg-card border-2 border-border">
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
              {PREDEFINED_EVENTS.map((predef) => (<Button key={predef.code} onClick={() => triggerEventNow(predef.code)} variant="outline" size="sm" className="justify-start">
        ...
        <Card className="bg-card border-2 border-border">
          ...
          <CardContent>
            {events.length > 0 ? (<div className="space-y-4">
                {events.map((event) => {
                const isActive = event.active &&
                    new Date(event.startsAt) <= new Date() &&
                    new Date(event.endsAt) >= new Date();
                return (<div key={event.id} className={`p-4 rounded border-2 ${isActive ? 'border-accent bg-accent/10' : 'border-border'}`}>
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-1">
                            <h3 className="font-bold text-text">{event.name}</h3>
                            {isActive && (<span className="text-xs px-2 py-0.5 bg-accent text-white rounded">
                                ACTIVE
                              </span>)}
                          </div>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex items-center gap-2">
                            ...
                            <Switch id={`toggle-${event.id}`} checked={event.active} onCheckedChange={() => toggleEventActive(event.id, event.active)}/>
                          </div>
                        </div>
                      </div>
                    </div>);
