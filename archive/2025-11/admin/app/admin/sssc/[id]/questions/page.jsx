    ...
    return (<Toast.Provider>
      <div className="p-6 space-y-4">
        ...
        <div className="border p-4">
          ...
          {showDupes && (<div className="mt-2 text-sm">
              {dupes.length === 0 && <p>No duplicates found.</p>}
              {dupes.map(d => (<div key={d.normalizedText} className="mb-2">
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map(r => (<Row key={r.id} row={r} onSave={saveRow} selected={!!selected[r.id]} onSelect={v => setSelected(s => ({ ...s, [r.id]: v }))}/>))}
          </tbody>
        </table>
      </div>
      <Toast.Viewport className="fixed bottom-4 right-4 flex flex-col gap-2"/>
      {toasts.map(t => (<Toast.Root key={t.id} duration={3000} onOpenChange={open => { if (!open)
            setToasts(ts => ts.filter(x => x.id !== t.id)); }}>
          <Toast.Title>{t.title}</Toast.Title>
        </Toast.Root>))}
    </Toast.Provider>);
}
function Row({ row, onSave, selected, onSelect, }) {
    ...
    return (<tr className="border-b align-top">
      ...
      <td className="py-2">
        <textarea className="w-full border rounded p-2" rows={2} value={text} onChange={e => setText(e.target.value)}/>
      ...
      <td className="py-2">
        <button className={`px-3 py-1 rounded ${dirty ? 'bg-black text-white' : 'bg-gray-200'}`} disabled={!dirty} onClick={() => onSave(row.id, { text, difficulty, approved })}>
          Save
        </button>
      </td>
    </tr>);
