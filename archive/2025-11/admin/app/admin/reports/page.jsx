...
import { Flag, AlertTriangle, User, MessageSquare, HelpCircle, Shield, Clock, CheckCircle, XCircle, Eye, } from 'lucide-react';
...
const TypeBadge = ({ type }) => {
    ...
    const { label, color, icon: Icon } = config[type] || config.USER;
    return (<Badge className={`${color} flex items-center gap-1`}>
      <Icon className="h-3 w-3"/>
      {label}
    </Badge>);
              ...
              {reports.map((report) => (<TableRow key={report.id}>
                  ...
                  <TableCell>
                    <div className="max-w-xs">
                      ...
                      {report.description && (<div className="text-sm text-muted-foreground truncate">
                          {report.description}
                        </div>)}
                    </div>
                  </TableCell>
                  ...
                  <TableCell>
                    <div className="max-w-xs">
                      ...
                      {report.reportedQuestion && (<div className="truncate" title={report.reportedQuestion.text}>
                          {report.reportedQuestion.text}
                        </div>)}
                      {report.reportedMessage && (<div className="truncate" title={report.reportedMessage.content}>
