      ...
      <Card>
        ...
        <CardContent>
          <table className="w-full">
            ...
            .map(([label, stats]) => (<tr key={label} className="border-b">
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2 max-h-[400px] overflow-y-auto">
            {logs.slice(-50).map((log, index) => (<div key={index} className="flex items-center justify-between p-2 border-b font-mono text-sm">
                <span className="text-subtle">{log.timestamp}</span>
                <span className="font-medium">{log.label}</span>
                <span className={`font-bold ${log.duration > 200
                ? 'text-red-400'
                : log.duration > 100
                    ? 'text-yellow-400'
                    : 'text-green-400'}`}>
