...
import { Loader2, Plus, Edit, Trash2, Play, BarChart3, Calculator } from "lucide-react";
import { apiFetch } from "@/lib/apiBase";
import { toast } from "sonner";
import { EnemyRegion, StatPreset, STAT_PRESETS, REGION_MULTIPLIERS, TIER_MULTIPLIERS } from "@/lib/rpg/enemy/types";
...
export default function AdminEnemiesPage() {
    ...
    const handleTestFight = async (enemyId) => {
        try {
            ...
            if (res.ok) {
                const result = res.data.fightResult;
                toast.success(`${result.result.toUpperCase()} in ${result.rounds} rounds`);
            }
            else {
                throw new Error(res.error || "Test fight failed");
            }
        }
        catch (error) {
    ...
    const handleSimulate = async (enemyId) => {
        try {
    ...
    const handleDelete = async (enemyId) => {
          ...
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <Input type="number" value={formData.power} onChange={(e) => setFormData({ ...formData, power: parseInt(e.target.value) || 0 })}/>
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Defense</label>
                <Input type="number" value={formData.defense} onChange={(e) => setFormData({ ...formData, defense: parseInt(e.target.value) || 0 })}/>
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Max HP</label>
                <Input type="number" value={formData.maxHp} onChange={(e) => setFormData({ ...formData, maxHp: parseInt(e.target.value) || 1 })}/>
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Rarity</label>
                <select className="w-full px-3 py-2 border rounded-md" value={formData.rarity} onChange={(e) => setFormData({ ...formData, rarity: e.target.value })}>
              ...
              <div>
                ...
                <select className="w-full px-3 py-2 border rounded-md" value={formData.region} onChange={(e) => setFormData({ ...formData, region: e.target.value })}>
                  {Object.values(EnemyRegion).map((r) => (<option key={r} value={r}>{r.charAt(0).toUpperCase() + r.slice(1)}</option>))}
                </select>
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Stat Preset</label>
                <select className="w-full px-3 py-2 border rounded-md" value={formData.statPreset} onChange={(e) => {
                ...
                setFormData({
                  ...
                  {Object.values(StatPreset).map((p) => (<option key={p} value={p}>{STAT_PRESETS[p].displayName}</option>))}
                </select>
              </div>
              <div>
            ...
            <div className="mt-4 space-y-2">
              <div className="flex items-center justify-between">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => setShowStatPreview(!showStatPreview)}>
              ...
              <div className="grid grid-cols-4 gap-4">
                <div>
                  ...
                  <Input type="number" value={formData.baseStats.hp} onChange={(e) => setFormData({
                ...formData,
                ...
                <div>
                  ...
                  <Input type="number" value={formData.baseStats.atk} onChange={(e) => setFormData({
                ...formData,
                ...
                <div>
                  ...
                  <Input type="number" value={formData.baseStats.def} onChange={(e) => setFormData({
                ...formData,
                ...
                <div>
                  ...
                  <Input type="number" value={formData.baseStats.speed} onChange={(e) => setFormData({
                ...formData,
            ...
            {showStatPreview && (<div className="mt-4 p-4 bg-muted rounded-lg">
                <h4 className="text-sm font-semibold mb-2">Stat Calculation Preview</h4>
                {(() => {
                    const preview = previewStatCalculation(formData.baseStats, formData.rarity, formData.region);
                    const regionMult = REGION_MULTIPLIERS[formData.region];
                    const tierMult = TIER_MULTIPLIERS[formData.rarity];
            ...
            <div className="mt-4 space-y-2">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <Input type="number" value={formData.lootTable.gold.min} onChange={(e) => setFormData({
                ...formData,
                lootTable: {
                    ...formData.lootTable,
                ...
                <div>
                  ...
                  <Input type="number" value={formData.lootTable.gold.max} onChange={(e) => setFormData({
                ...formData,
                lootTable: {
                    ...formData.lootTable,
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {enemies.map((enemy) => (<tr key={enemy.id} className="border-b">
                    ...
                    <td className="py-2">
                      <div className="flex gap-2">
                        <Button size="sm" variant="outline" onClick={() => handleTestFight(enemy.id)} title="Test Fight">
                          <Play className="h-3 w-3"/>
                        </Button>
                        <Button size="sm" variant="outline" onClick={() => handleSimulate(enemy.id)} title="Simulate 1000 Fights">
                ...
                setShowForm(true);
                        ...
                        <Button size="sm" variant="outline" onClick={() => handleDelete(enemy.id)} title="Delete">
