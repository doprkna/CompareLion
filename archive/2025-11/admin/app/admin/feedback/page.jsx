...
import { RefreshCw, Search, Filter, CheckCircle, Circle, Bug, Lightbulb, Heart, Calendar, User, MessageSquare, ChevronDown, ChevronUp } from 'lucide-react';
export default function AdminFeedbackPage() {
    ...
    const updateStatus = async (feedbackId, newStatus) => {
        try {
            ...
            // Update local state
            setFeedback(prev => prev.map(item => item.id === feedbackId
    ...
    return (<div className="p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4">
            ...
            {/* Category */}
            <select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)} className="px-3 py-2 border border-border rounded-md bg-bg">
            ...
            {/* Status */}
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="px-3 py-2 border border-border rounded-md bg-bg">
      ...
      <div className="space-y-4">
        {filteredFeedback.length === 0 ? (<Card>
          ...
          </Card>) : (filteredFeedback.map((item) => (<Card key={item.id} className="overflow-hidden">
              <CardContent className="p-4">
                {/* Header Row */}
                <div className="flex items-start justify-between gap-4 mb-3">
                  ...
                  <div className="flex items-center gap-2">
                    {item.status === 'pending' && (<Button size="sm" variant="outline" onClick={() => updateStatus(item.id, 'reviewed')} disabled={updatingStatus === item.id}>
                        <CheckCircle className="h-4 w-4 mr-1"/>
                        Mark Reviewed
                      </Button>)}
                    {item.status === 'reviewed' && (<Button size="sm" variant="outline" onClick={() => updateStatus(item.id, 'pending')} disabled={updatingStatus === item.id}>
                        <Circle className="h-4 w-4 mr-1"/>
                        Mark Pending
                      </Button>)}
                    <Button size="sm" variant="ghost" onClick={() => setExpandedItem(expandedItem === item.id ? null : item.id)}>
                ...
                {/* Expanded Details */}
                {expandedItem === item.id && (<div className="mt-4 pt-4 border-t border-border space-y-3">
                    <div>
