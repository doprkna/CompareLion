...
async function getItemsWithStats() {
    ...
    return (<div className="min-h-screen bg-bg p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <Link href="/admin" className="px-4 py-2 bg-accent text-white rounded hover:opacity-90 transition">
            Back to Admin
          </Link>
        </div>
        ...
        {items.length === 0 ? (<Card>
            ...
            {items.map((item) => (<Card key={item.id} className="hover:border-accent transition-colors">
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="text-4xl">{item.icon || 'ðŸ“¦'}</div>
                    <div className="flex flex-col gap-1">
                      {item.rarity && (<Badge className={'${rarityColors[item.rarity] || g-gray-500} text-white text-xs'}>
                          {item.rarity}
                        </Badge>)}
                      {item.inShop && (<Badge className="bg-green-500 text-white text-xs">SHOP</Badge>)}
                    </div>
                  </div>
