...
export default function SeedsAdmin() {
    ...
    const getStatusBadge = (status) => {
        ...
        return (<span className={`px-2 py-1 rounded text-xs font-semibold ${statusColors[status] || statusColors.PENDING}`}>
        {status}
      </span>);
    };
    if (initialLoading) {
    ...
    return (<div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 border rounded-lg p-6">
        ...
        <div className="flex flex-col sm:flex-row gap-4">
          <div className="flex-1">
            ...
            <select className="w-full border rounded-md px-3 py-2 bg-background text-foreground" value={lang} onChange={(e) => setLang(e.target.value)} disabled={loading}>
              {GEN_CONFIG.LANGUAGES.map((l) => (<option key={l} value={l}>
                  {l.toUpperCase()}
                </option>))}
            </select>
          </div>
          ...
          <div className="flex items-end">
            <button onClick={trigger} disabled={loading} className="px-6 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed transition-colors font-medium">
              {loading ? "Creating..." : "Create Batch"}
            </button>
          </div>
        </div>

        {msg && (<div className="mt-4 p-3 bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded text-sm text-green-800 dark:text-green-200">
            {msg}
          </div>)}
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
              {batches.length === 0 ? (<tr>
                  <td colSpan={9} className="p-8 text-center text-muted-foreground">
                ...
                </tr>) : (batches.map((b) => (<tr key={b.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                    <td className="p-3">
                    ...
                    <td className="p-3 text-center">
                      <div className="flex flex-col items-center">
                        ...
                        {b.targetCount > 0 && (<div className="w-full max-w-[100px] bg-gray-200 dark:bg-gray-700 rounded-full h-1.5 mt-1">
                            <div className="bg-blue-600 dark:bg-blue-400 h-1.5 rounded-full transition-all" style={{ width: `${(b.processed / b.targetCount) * 100}%` }}/>
                          </div>)}
                      </div>
                    </td>
                    <td className="p-3 text-center">
                    ...
                    <td className="p-3 text-center">
                      {b.failed > 0 && (<button onClick={() => retryFailed(b.id)} disabled={loading} className="px-3 py-1 text-xs bg-orange-600 text-white rounded hover:bg-orange-700 disabled:opacity-50 transition-colors">
                          Retry Failed
                        </button>)}
                    </td>
                  </tr>)))}
      ...
      {batches.some((b) => b.status === "RUNNING" || b.status === "PENDING") && (<AutoRefresh onRefresh={refresh} interval={5000}/>)}
    </div>);
}
/**
 ...
 */
function AutoRefresh({ onRefresh, interval }) {
    useEffect(() => {
