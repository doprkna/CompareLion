...
import { Activity, Database, Wifi, Bug, GitBranch, Clock, RefreshCw, ExternalLink, Server, AlertCircle, CheckCircle, XCircle, Minus } from 'lucide-react';
function StatusIndicator({ status, label }) {
    ...
    return (<div className={`flex items-center gap-2 px-3 py-1 rounded-md border ${bg} ${border}`}>
    ...
    return (<div className="p-8 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-3">
          {lastUpdate && (<span className="text-sm text-gray-500">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <StatusIndicator status={dbStatus} label={health.db.ok ? 'Connected' : 'Error'}/>
            {health.db.ok && health.db.latencyMs !== undefined && (<div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-gray-500">Latency:</span>
                  <span className="font-mono font-medium">{health.db.latencyMs}ms</span>
                </div>
              </div>)}
            {health.db.error && (<div className="text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">
                {health.db.error}
              </div>)}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <StatusIndicator status={redisStatus} label={!health.redis.present
            ? 'Not Configured'
            : health.redis.ok
                ? 'Connected'
                : 'Error'}/>
            {health.redis.error && (<div className="text-xs text-red-600 bg-red-50 p-2 rounded border border-red-200">
                {health.redis.error}
              </div>)}
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <StatusIndicator status={sentryStatus} label={health.sentry.enabled ? 'Enabled' : 'Disabled'}/>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            <div className="space-y-2 text-sm">
              {health.commit && (<div className="flex justify-between">
            ...
            <div className="space-y-2 text-sm">
              ...
              <div className="space-y-1 pt-2 border-t">
                ...
                <div className="flex flex-wrap gap-1">
                  {health.features.enableRedis && (<Badge variant="secondary" className="text-xs">Redis</Badge>)}
                  {health.features.enableSentry && (<Badge variant="secondary" className="text-xs">Sentry</Badge>)}
                  {!health.features.enableRedis && !health.features.enableSentry && (<span className="text-xs text-gray-400">None active</span>)}
                </div>
              </div>
            </div>
          </CardContent>
