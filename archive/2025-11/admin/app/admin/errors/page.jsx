...
import { RefreshCw, AlertTriangle, AlertCircle, Info, CheckCircle, TrendingUp, Clock, Filter, } from 'lucide-react';
...
export default function AdminErrorsPage() {
    ...
    const loadErrors = async () => {
        try {
            ...
            if (filterResolved !== null)
                params.set('resolved', String(filterResolved));
            ...
            setStats({
                ...
                last24h: data.errors.filter((e) => new Date(e.lastSeen).getTime() > oneDayAgo).length,
            });
        }
        catch (err) {
    ...
    return (<div className="p-6 space-y-6">
        ...
        <CardContent className="p-4">
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={filterSeverity || ''} onChange={(e) => setFilterSeverity(e.target.value || null)} className="text-sm border rounded px-2 py-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <select value={filterResolved === null ? 'all' : filterResolved ? 'resolved' : 'unresolved'} onChange={(e) => {
      ...
      <Card>
        ...
        <CardContent>
          {error && (<div className="text-red-500 mb-4 p-4 bg-red-50 rounded">
              {error}
            </div>)}
          
          {errors.length === 0 ? (<div className="text-center py-12 text-text-secondary">
              ...
              {errors.map((err) => {
                const SeverityIcon = SEVERITY_ICONS[err.severity] || AlertCircle;
                const severityColor = SEVERITY_COLORS[err.severity] || SEVERITY_COLORS.error;
                return (<div key={err.id} className={`p-4 rounded-lg border ${severityColor} ${err.resolved ? 'opacity-60' : ''}`}>
                      ...
                      <div className="flex items-start gap-3 flex-1">
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center gap-2 mb-1">
                            ...
                            {err.resolved && (<Badge variant="outline" className="text-xs bg-green-50">
                                Resolved
                              </Badge>)}
                          </div>
                          <p className="text-sm mb-2 break-words">{err.message}</p>
                          {err.page && (<p className="text-xs text-text-secondary">
