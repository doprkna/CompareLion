            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div>
                ...
                <select className="w-full px-3 py-2 border rounded-md" value={formData.type} onChange={(e) => setFormData({ ...formData, type: e.target.value })}>
                  {Object.values(MissionType).map((t) => (<option key={t} value={t}>{getMissionTypeDisplayName(t)}</option>))}
                </select>
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Objective Type</label>
                <select className="w-full px-3 py-2 border rounded-md" value={formData.objectiveType} onChange={(e) => setFormData({ ...formData, objectiveType: e.target.value })}>
                  {Object.values(ObjectiveType).map((ot) => (<option key={ot} value={ot}>{getObjectiveTypeDisplayName(ot)}</option>))}
                </select>
              </div>
              <div>
                <label className="text-sm font-medium mb-1 block">Target Value</label>
                <Input type="number" value={formData.targetValue} onChange={(e) => setFormData({ ...formData, targetValue: parseInt(e.target.value) || 1 })}/>
              </div>
              ...
              <div>
                ...
                <Input type="number" value={formData.reward.xp || 0} onChange={(e) => setFormData({
                ...formData,
              ...
              <div>
                ...
                <Input type="number" value={formData.reward.gold || 0} onChange={(e) => setFormData({
                ...formData,
              ...
              <div>
                ...
                <Input type="number" value={formData.reward.battlepassXP || 0} onChange={(e) => setFormData({
                ...formData,
              ...
              <div className="flex items-center gap-2">
                <input type="checkbox" checked={formData.isRepeatable} onChange={(e) => setFormData({ ...formData, isRepeatable: e.target.checked })}/>
        ...
        <CardContent>
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {missions.map((mission) => (<tr key={mission.id} className="border-b">
                    ...
                    <td className="py-2">
                      {mission.isActive ? (<span className="px-2 py-1 rounded text-xs bg-green-900/30 text-green-400">Active</span>) : (<span className="px-2 py-1 rounded text-xs bg-muted">Inactive</span>)}
                    </td>
                    <td className="py-2">
                      <div className="flex gap-2">
                ...
                setShowForm(true);
                        ...
                        <Button size="sm" variant="outline" onClick={() => handleDelete(mission.id)} title="Delete">
                          <Trash2 className="h-3 w-3"/>
                        </Button>
                      </div>
                    </td>
                  </tr>))}
                {missions.length === 0 && (<tr>
