...
const SYSTEMS = [
    {
        ...
        route: 'economy',
        models: ['economyStat', 'treasury', 'taxTransaction', 'dynamicPrice'],
    },
    {
        name: 'Creator Economy',
        route: 'creator',
        models: ['creatorWallet', 'creatorTransaction', 'payoutPool', 'engagementMetric'],
    },
    {
        name: 'Localization',
        route: 'localization',
        models: ['translationKey', 'languagePreference', 'language'],
    },
    {
        name: 'Regional Events',
        route: 'regional',
        models: ['regionalEvent', 'regionConfig', 'regionSchedule'],
    },
    {
        name: 'Timezones',
        route: 'timezones',
    ...
    },
    {
        ...
        route: 'lore',
        models: ['loreEntry', 'chronicle', 'narrativeQuest', 'worldChronicle'],
    },
    {
        name: 'Moderation',
        route: 'moderation',
        models: ['moderationAction', 'moderationReport', 'report'],
    },
    {
        name: 'Subscription',
        route: 'subscription',
        models: ['subscription', 'userSubscription', 'subscriptionPlan'],
    },
];
async function getSystems() {
    try {
        const systemsWithCounts = await Promise.all(SYSTEMS.map(async (system) => {
            let totalCount = 0;
            let status = 'active';
            try {
...
export default async function DevLabPage() {
    ...
    return (<div className="min-h-screen bg-bg p-6">
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card>
          <CardHeader>
            <div className="flex items-center justify-between">
              ...
              <Link href="/admin/flags" className="text-sm text-blue-500 hover:text-blue-400 transition-colors">
                Manage →
              </Link>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
              {Object.entries(flags).map(([key, value]) => (<div key={key} className="flex flex-col gap-1">
                  <span className="text-xs text-subtle truncate">{key}</span>
                  <Badge variant={typeof value === 'boolean'
                ? (value ? 'default' : 'outline')
                : 'secondary'} className="w-fit">
                    {String(value)}
                  </Badge>
                </div>))}
            </div>
        ...
        {/* Error State */}
        {error && (<div className="bg-red-500/10 border-2 border-red-500 rounded-lg p-4">
            <p className="text-red-500 font-medium">Error: {error}</p>
          </div>)}

        {/* Systems Grid */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {systems.length > 0 ? (systems.map((system) => (<AdminSystemCard key={system.route} name={system.name} route={system.route} modelCount={system.modelCount} status={system.status}/>))) : (<div className="col-span-full text-center text-subtle py-12">
