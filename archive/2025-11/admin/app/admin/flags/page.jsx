...
export default function AdminFlagsPage() {
    ...
    const handleToggle = (flagKey, value) => {
        if (!isDev) {
    ...
    const renderFlagValue = (key, value) => {
        if (typeof value === 'boolean') {
            return (<Switch checked={value} onCheckedChange={(checked) => handleToggle(key, checked)} disabled={!isDev || key === 'environment'}/>);
        }
        // For non-boolean values (like environment), just display
        return (<Badge variant="outline" className="font-mono">
        {String(value)}
      </Badge>);
    };
    return (<div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-subtle mt-1">
            Centralized feature toggle system • {isDev ? 'Development Mode' : 'Production Mode'}
          </p>
        </div>
      ...
      {/* Environment Warning */}
      {!isDev && (<Card className="border-yellow-500/50 bg-yellow-500/10">
      ...
      {/* Flags by Category */}
      {Object.entries(flagCategories).map(([category, flagKeys]) => (<Card key={category}>
          <CardHeader>
          ...
          <CardContent>
            <div className="space-y-4">
              {flagKeys.map((flagKey) => {
                ...
                return (<div key={flagKey} className="flex items-center justify-between p-4 border rounded-lg">
