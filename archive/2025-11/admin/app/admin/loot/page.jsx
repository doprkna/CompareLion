...
export default function AdminLootPage() {
    ...
    async function handleSeed() {
        ...
        try {
            ...
            if (res.ok) {
                ...
                throw new Error(res.error || 'Seed failed');
            }
        }
        catch (error) {
            toast({
    ...
    async function simulateRoll(tableId) {
    ...
    return (<div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <Card className="bg-gray-800 border-gray-700 mb-6">
        <CardContent className="p-4">
          <div className="flex gap-4">
            <Button onClick={handleSeed} disabled={seeding} className="bg-blue-600 hover:bg-blue-700 text-white">
              {seeding ? (<>
            ...
            <Button onClick={loadTables} disabled={loading} variant="outline" className="bg-gray-700 border-gray-600 text-white hover:bg-gray-600">
      ...
      {/* Simulation Result */}
      {simResult && (<Card className="bg-gray-800 border-gray-700 mb-6">
          <CardContent className="p-4">
            <h3 className="text-lg font-bold text-white mb-2">Simulation Result</h3>
            {simResult.dropped ? (<div className="flex items-center gap-4">
              ...
              </div>) : (<p className="text-gray-400">No loot drop</p>)}
            <Button onClick={() => setSimResult(null)} variant="outline" size="sm" className="mt-2">
              Close
            </Button>
          </CardContent>
        </Card>)}
      ...
      <Card className="bg-gray-800 border-gray-700">
        ...
        <CardContent>
          {loading ? (<div className="flex items-center justify-center py-12">
            ...
            </div>) : tables.length === 0 ? (<div className="text-center py-12 text-gray-400">
              <Package className="w-16 h-16 mx-auto mb-4 opacity-50"/>
              <p>No loot tables found. Seed base tables to get started.</p>
            </div>) : (<div className="space-y-4">
              {tables.map((table) => (<Card key={table.id} className="bg-gray-900 border-gray-700">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <Button onClick={() => simulateRoll(table.id)} disabled={simulating === table.id} size="sm" variant="outline" className="bg-gray-700 border-gray-600 text-white hover:bg-gray-600">
                        {simulating === table.id ? (<>
                            <Loader2 className="w-4 h-4 mr-2 animate-spin"/>
                            Rolling...
                          </>) : (<>
                    ...
                    <div className="mb-3">
                      ...
                      <div className="flex gap-4 flex-wrap">
                        {Object.entries(table.weights).map(([rarity, weight]) => (<div key={rarity} className="text-sm">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {Object.entries(table.items).map(([rarity, itemIds]) => (<div key={rarity} className="text-sm">
