    ...
    return (<div className="container max-w-6xl mx-auto py-8 px-4">
      ...
      <Card>
        <CardHeader className="flex flex-row items-center justify-between pb-3">
          ...
          <Button onClick={showLogs ? () => setShowLogs(false) : fetchLogs} variant="outline" size="sm">
            {showLogs ? 'Hide Logs' : 'Show Logs'}
          </Button>
        </CardHeader>
        {showLogs && (<CardContent>
            {logs.length === 0 ? (<p className="text-sm text-muted-foreground text-center py-4">
                No logs yet
              </p>) : (<div className="space-y-2">
                {logs.map((log) => (<div key={log.id} className="flex items-center justify-between p-3 bg-muted rounded-lg text-sm">
