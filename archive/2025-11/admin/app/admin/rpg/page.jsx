...
export default function AdminRPGPage() {
    ...
    const generateEnemy = async () => {
        try {
            setLoading(true);
            const response = await apiFetch('/api/admin/rpg/generate-enemy', {
                method: 'POST',
                body: JSON.stringify({
                    playerLevel,
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input type="number" min="1" max="100" value={playerLevel} onChange={(e) => setPlayerLevel(parseInt(e.target.value) || 1)} className="w-full px-3 py-2 bg-bg border border-border rounded-md text-text"/>
            </div>
            <div>
              <label className="text-sm font-medium text-text mb-2 block">
                Tier (Optional)
              </label>
              <select value={selectedTier} onChange={(e) => setSelectedTier(e.target.value)} className="w-full px-3 py-2 bg-bg border border-border rounded-md text-text">
          ...
          <Button onClick={generateEnemy} disabled={loading} className="w-full">
            {loading ? (<>
            ...
            <CardHeader>
              <CardTitle className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  {enemy.variant && (<span className={`text-sm ${getVariantColor(enemy.variant)}`}>
                      {enemy.variant}
                    </span>)}
                </div>
              </CardTitle>
