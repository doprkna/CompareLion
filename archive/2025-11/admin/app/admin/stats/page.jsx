...
import { getXPForLevel, getLevelFromXP } from '@/lib/levelCurve';
export default function AdminStatsPage() {
    ...
    function simulateLevelUp(startLevel, xpGained) {
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <Input type="number" min="1" max="100" value={testLevel} onChange={(e) => setTestLevel(parseInt(e.target.value) || 1)}/>
            ...
            <div>
              ...
              <Input type="number" min="0" value={testXP} onChange={(e) => setTestXP(parseInt(e.target.value) || 0)}/>
              <div className="mt-2 text-sm text-muted-foreground">
          ...
          <div className="mt-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-2 text-xs">
              {[1, 2, 5, 10, 20].map((level) => (<div key={level} className="p-2 border rounded">
        ...
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <Input type="number" min="1" max="99" defaultValue="5" id="sim-start-level"/>
            </div>
            <div>
              <label className="text-sm font-medium mb-2 block">XP to Gain</label>
              <Input type="number" min="1" defaultValue="1000" id="sim-xp-gain"/>
            </div>
            <div className="flex items-end">
              <Button onClick={() => {
            const startLevelInput = document.getElementById('sim-start-level');
            const xpGainInput = document.getElementById('sim-xp-gain');
            ...
            toast.success(`Level ${result.startLevel} → ${result.newLevel} (+${result.levelsGained} levels, +${result.pointsGained} attribute points)`);
        }}>
                Simulate
              </Button>
            </div>
