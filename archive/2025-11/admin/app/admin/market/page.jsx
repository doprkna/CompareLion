...
export default function AdminMarketPage() {
    ...
    async function handleCleanup() {
        ...
        try {
            ...
            if (res.ok) {
                toast({
                    title: 'Success',
                    description: `Cleaned up ${res.data.restoredCount} expired listings`,
                });
                await loadListings();
            }
            else {
    ...
    return (<div className="container mx-auto px-4 py-8 max-w-6xl">
      ...
      <Card className="bg-gray-800 border-gray-700 mb-6">
        <CardContent className="p-4">
          <div className="flex gap-4">
            ...
            <Button onClick={loadListings} disabled={loading} variant="outline" className="bg-gray-700 border-gray-600 text-white hover:bg-gray-600">
              <RefreshCw className="w-4 h-4 mr-2"/>
              Refresh
            </Button>
          </div>
      ...
      <Card className="bg-gray-800 border-gray-700 mb-6">
        <CardContent className="p-4">
          <div className="flex gap-4">
            <Input placeholder="Search by User ID..." value={searchUserId} onChange={(e) => setSearchUserId(e.target.value)} className="bg-gray-900 border-gray-700 text-white"/>
            <Input placeholder="Search by Listing ID..." value={searchFightId} onChange={(e) => setSearchFightId(e.target.value)} className="bg-gray-900 border-gray-700 text-white"/>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="bg-gray-800 border-gray-700">
        ...
        <CardContent>
          {loading ? (<div className="flex items-center justify-center py-12">
            ...
            </div>) : listings.length === 0 ? (<div className="text-center py-12 text-gray-400">
              ...
              <table className="w-full text-left">
                ...
                <tbody>
                  {listings
                .filter(listing => {
                if (searchUserId && !listing.sellerId.includes(searchUserId))
                    return false;
                if (searchFightId && !listing.id.includes(searchFightId))
                    return false;
                return true;
            })
                .map((listing) => (<tr key={listing.id} className="border-b border-gray-700">
                    ...
                    : listing.status === 'sold'
                        ...
                        <td className="p-2">
                          {listing.status === 'active' && (<Button onClick={() => handleDeactivate(listing.id)} variant="destructive" size="sm">
