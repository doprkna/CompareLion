...
import { Search, Plus, Eye, AlertCircle, CheckCircle, Clock, BarChart3, RefreshCcw, LayoutDashboard, BookOpen, } from 'lucide-react';
...
const StatusBadge = ({ status }) => {
    ...
    const { label, color, icon: Icon } = config[status] || config.empty;
...
const CategoryCard = ({ category }) => {
    return (<Card className="hover:shadow-md transition-shadow">
      ...
      <CardContent className="space-y-4">
        ...
        {category.status !== 'complete' && (<div className="text-sm text-muted-foreground">
...
export default function CategoryHealthPage() {
    ...
    const { data, error, isLoading, mutate } = useSWR('/api/categories/health', fetcher, {
        revalidateOnFocus: false,
        revalidateOnReconnect: false,
    });
    const [searchTerm, setSearchTerm] = useState('');
    const [statusFilter, setStatusFilter] = useState('all');
    const [sortBy, setSortBy] = useState('completion-asc');
    useEffect(() => {
    ...
    if (error) {
        ...
        <Card className="bg-card border-2 border-border max-w-lg">
          <CardContent className="p-6">
            <EmptyState icon={AlertCircle} title="Category Health Error" description={`Failed to load category data: ${error.message}. Please ensure you are logged in as an ADMIN and the backend is running.`} action={<div className="flex gap-3">
    ...
    return (<div className="min-h-screen bg-bg p-6 text-text">
      <div className="max-w-7xl mx-auto space-y-8">
        {/* Header */}
        ...
        {/* Summary Cards */}
        {data?.summary && (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Card className="p-4">
        ...
        {filteredCategories.length === 0 && (<Card className="p-8">
            <EmptyState icon={BookOpen} title="No Categories Found" description="No categories match your current filters. Try adjusting your search or filter criteria."/>
          </Card>)}
      </div>
    </div>);
}
