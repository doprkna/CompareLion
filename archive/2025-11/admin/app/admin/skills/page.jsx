...
export default function AdminSkillsPage() {
    ...
    async function handleUnlockSkill(userId, skillId) {
        try {
    ...
    return (<div className="container mx-auto p-6 space-y-6">
      ...
      {skills.length === 0 ? (<Card className="border-2 border-dashed">
          ...
          {skills.map((skill) => {
                ...
                return (<Card key={skill.id} className={`${typeColor} border-2`}>
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <div>
                          {isEditing ? (<Input value={editForm.name || ''} onChange={(e) => setEditForm({ ...editForm, name: e.target.value })} className="text-lg font-semibold"/>) : (<CardTitle className="text-lg">{skill.name}</CardTitle>)}
                      ...
                      <div className="flex items-center gap-2 mt-2">
                        ...
                        {skill.cooldown && (<span className="text-xs px-2 py-0.5 rounded bg-gray-200">
                            CD: {skill.cooldown}
                          </span>)}
                      </div>
                    </div>
                  ...
                  {isEditing ? (<>
                      ...
                      <div className="grid grid-cols-2 gap-2">
                        <div className="space-y-1">
                          ...
                          <Input type="number" value={editForm.power || 0} onChange={(e) => setEditForm({ ...editForm, power: parseInt(e.target.value) || 0 })}/>
                        </div>
                        {skill.type === 'active' && (<div className="space-y-1">
                            <label className="text-xs font-medium">Cooldown</label>
                            <Input type="number" value={editForm.cooldown || 0} onChange={(e) => setEditForm({ ...editForm, cooldown: parseInt(e.target.value) || null })}/>
                          </div>)}
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs font-medium">Icon</label>
                        <Input value={editForm.icon || ''} onChange={(e) => setEditForm({ ...editForm, icon: e.target.value })} placeholder="⚔️"/>
                      </div>
                      <div className="flex gap-2">
                        <Button size="sm" onClick={() => saveEdit(skill.id)} className="flex-1">
                      ...
                      <div className="pt-2 border-t">
                        ...
                        <div className="flex gap-2">
                          <Input placeholder="User ID" className="text-xs flex-1" id={`unlock-${skill.id}`}/>
                          <Button size="sm" variant="outline" onClick={() => {
                            const input = document.getElementById(`unlock-${skill.id}`);
