      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={eventTypeData}>
                ...
                <Bar dataKey="value" fill="#3b82f6" name="Events">
                  {eventTypeData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[entry.displayName] || '#3b82f6'}/>))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <ResponsiveContainer width="100%" height={300}>
              <PieChart>
                <Pie data={eventTypeData} cx="50%" cy="50%" labelLine={false} label={(entry) => `${entry.name}: ${entry.value}`} outerRadius={100} fill="#8884d8" dataKey="value">
                  {eventTypeData.map((entry, index) => (<Cell key={`cell-${index}`} fill={COLORS[entry.displayName] || '#3b82f6'}/>))}
                </Pie>
                <Tooltip />
              </PieChart>
      ...
      <Card>
        ...
        <CardContent>
          <div className="space-y-2">
            {metrics?.recentEvents && metrics.recentEvents.length > 0 ? (metrics.recentEvents.slice(0, 10).map((event, index) => (<div key={index} className="flex items-center justify-between p-3 bg-bg rounded-lg border border-border">
                  <div className="flex items-center gap-3">
                    <div className="w-3 h-3 rounded-full" style={{ backgroundColor: COLORS[event.name] || '#3b82f6' }}/>
                    <span className="font-medium">{event.name.replace(/_/g, ' ')}</span>
                  </div>
                  <span className="text-sm text-text-secondary">
                    {new Date(event.timestamp).toLocaleString()}
                  </span>
                </div>))) : (<div className="text-center text-text-secondary py-8">
