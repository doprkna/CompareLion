 ...
 */
async function aggregateByDateAndType(date, type, context) {
 ...
 */
export async function aggregateTelemetryDaily() {
    ...
    try {
        ...
        for (const type of types) {
            ...
            if (type === "page_view" || type === "action") {
                ...
                contexts.forEach((c) => {
                    if (c.page)
                        uniqueContexts.add(c.page);
                    if (c.action)
                        uniqueContexts.add(c.action);
                });
 ...
 */
export async function getAggregatedMetrics(startDate, endDate, type) {
    const where = {
        date: {
            gte: startDate,
            lte: endDate,
