 ...
 */
export async function healStaleSessions() {
    ...
    try {
        ...
        if (deleted > 0) {
            ...
            captureMessage(`Cleaned up ${deleted} stale sessions`, { action: "heal_stale_sessions" }, ErrorSeverity.INFO);
        }
        return deleted;
    }
    catch (error) {
        logger.error("Failed to clean stale sessions:", error);
        throw error;
    }
}
/**
 ...
 */
export async function healStuckJobs() {
    ...
    try {
            ...
            try {
                ...
                for (const job of stuckJobs) {
                    ...
                    if (activeTime > fiveMinutes) {
                        ...
                        await job.moveToFailed(new Error("Job stuck - auto-failed by self-healing"), "0");
                        totalHealed++;
        ...
        if (totalHealed > 0) {
            captureMessage(`Healed ${totalHealed} stuck jobs`, { action: "heal_stuck_jobs" }, ErrorSeverity.WARNING);
        }
        return totalHealed;
    }
    catch (error) {
        logger.error("Failed to heal stuck jobs:", error);
        throw error;
    }
}
/**
 ...
 */
export async function cleanOrphanedRecords() {
    ...
    try {
        ...
        if (totalCleaned > 0) {
            ...
            captureMessage(`Cleaned ${totalCleaned} orphaned records`, { action: "clean_orphaned_records" }, ErrorSeverity.INFO);
        }
        return totalCleaned;
    }
    catch (error) {
 ...
 */
export async function runAllHealingRoutines() {
