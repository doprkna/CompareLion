...
export default function LegacyPage() {
    ...
    if (loading) {
        return (<div className="container mx-auto p-4 flex items-center justify-center min-h-screen">
    ...
    return (<div className="container mx-auto p-4 space-y-6 max-w-4xl">
      ...
      {error && (<div className="bg-destructive/10 text-destructive rounded-lg p-4">
          Error: {error}
        </div>)}
      ...
      {current && (<>
          ...
          <Card className="bg-card border-border">
            ...
            <CardContent className="space-y-4">
              ...
              {current.currentGenerationRecord && (<div className="bg-muted rounded-lg p-3 border border-border">
              ...
              {current.allInheritedPerks && current.allInheritedPerks.length > 0 && (<div className="space-y-2">
                  <div className="text-sm font-semibold flex items-center gap-2">
                    <Sparkles className="w-4 h-4 text-purple-500"/>
                    All Inherited Perks
                  </div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {current.allInheritedPerks.map((perk, index) => (<div key={index} className="bg-muted rounded-lg p-2 border border-border text-sm">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold capitalize">{perk.type}:</span>
                          <span className="text-primary">{String(perk.value)}</span>
                          {perk.fromGeneration && (<span className="text-xs text-muted-foreground ml-auto">
              ...
              {current.canAscend && (<div className="flex justify-center pt-2">
                  <Button onClick={() => setShowAscendModal(true)} className="bg-yellow-600 hover:bg-yellow-700 text-white" size="lg">
                    <Crown className="w-5 h-5 mr-2"/>
                    Ascend to Next Generation
                  </Button>
                </div>)}

              {!current.canAscend && (<div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-3">
          ...
          {/* Ascend Modal */}
          <AscendModal open={showAscendModal} onClose={() => setShowAscendModal(false)} currentGeneration={current.currentGeneration || 1} prestigeCount={current.prestigeCount || 0} canAscend={current.canAscend || false} onAscended={handleAscended}/>
        </>)}
    </div>);
}
