...
export function useGlobalMood() {
  const [mood, setMood] = useState<any | null>(null);
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (!res.ok || !json?.success) throw new Error(json?.error || 'Failed to load global mood');
...
export function useMoodTheme(mood: any | null) {
  ...
  const getMoodText = useCallback(() => {
    if (!mood || !mood.dominantEmotion) return 'World feels calm today 🌿';
