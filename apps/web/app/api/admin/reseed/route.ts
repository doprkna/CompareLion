...
export const POST = safeAsync(async (req: NextRequest) => {
  console.log('ðŸ” [Reseed] Request received from admin...');
  ...
  const successItems = [];
  if (result.stats.users > 0) successItems.push(`ðŸ‘¥  users`);
  if (result.stats.achievements > 0) successItems.push(`ðŸ†  achievements`);
  if (result.stats.items > 0) successItems.push(`ðŸ“¦  items`);
  if (result.stats.questions > 0) successItems.push(`â“  questions`);
  if (result.stats.messages > 0) successItems.push(`ðŸ’¬  messages`);
  if (result.stats.notifications > 0) successItems.push(`ðŸ””  notifications`);
  if (result.stats.events > 0) successItems.push(`ðŸŒ  events`);
  if (result.stats.leaderboard > 0) successItems.push(`ðŸ…  leaderboard entries`);

  const detailedMessage = result.success
    ? `Database reseeded successfully!\\n\\nCreated: `
    : `Database partially reseeded ( errors)\\n\\nCreated: \\n\\nErrors: `;

  console.log(`\\nðŸ“Š [Reseed] Complete! Duration: `);
